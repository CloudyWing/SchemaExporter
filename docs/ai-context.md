...
AI context 是供 Agent 或自動化流程讀取的 Markdown 摘要，內容聚焦 schema metadata、物件清單、欄位、索引、routine signature、診斷資訊與差異摘要。
...
或 CLI 傳入 `--ai-context` 參數。

## 檔案命名

與主要 Excel 輸出檔案同名，後綴為 `.schema-context.md`。例如：

```text
TableSchema_prod_20241201_120000.xlsx
TableSchema_prod_20241201_120000.schema-context.md
```
