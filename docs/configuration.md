...
既有設定檔若仍包含 `GenerateAiContext`，讀取時會轉為 `GenerateSchemaSummary`；儲存後會輸出新欄位。各 artifacts 的定位與檔名後綴參閱 [artifacts.md](artifacts.md)。
...
| `SensitiveNamePatterns` | string[] | 內建常見敏感名稱規則 | 用來比對物件、欄位與 routine 名稱的 .NET regex。命中後會遮罩該項目的描述、預設值或 routine 定義等 metadata。 |
